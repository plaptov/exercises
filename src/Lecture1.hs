...
minmax x y z = twice max - twice min
    where
...
subString start end str = take (fixedEnd - fixedStart + 1) (drop fixedStart str)
    where
        fixedStart = max 0 start
        fixedEnd = max (-1) end
...
-}
strSum :: String -> Int
strSum str = sum (map (\s -> read s :: Int) (words str))
...
lowerAndGreater n list = show n ++ " is greater than " ++ show (count 0 (< n) list) ++ " elements and lower than " ++ show (count 0 (> n) list) ++ " elements"
    where
        count :: Int -> (Int -> Bool) -> [Int] -> Int
        count result predicate l
          | null l = result
          | predicate (head l) = count (result + 1) predicate (tail l)
          | otherwise = count result predicate (tail l)
